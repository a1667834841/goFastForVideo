...
func StartStudy(userName string, password string, videos []query.Video) {
	fmt.Println("开始学习。。。")
	for _, video := range videos {
		...
		Wg.Add(1)
		go learnServer(userName, password, video)

	}

}

func learnServer(userName, password string, video query.Video) {
	client := &http.Client{}
